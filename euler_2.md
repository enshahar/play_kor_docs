...
```scala
// Stream.from도 있으나, 여기서는 함수를 정의해 봤다.
...
성능 체크를 위해 print를 넣어보면, 결과가 참혹하다 내 PC(intel core-i5 2400 3.2GHz)에서 4~5분 정도가 걸렸다.
...
스크림을 공유해 보는 것이다. 해보면 속도가 빨라지긴 하지만, 여전히 그렇게 실용적이지는 않아 보인다.
...
1. 원래의 i번째 원소(타입은 `ReturnType`)를 계산하는 함수 `f(i:Int)`가 있다면, 이를 Stream[ReturnType]을 반환하는 
함수 `f_stream_fun(i:Int)`로 변경한다. 일단은 `f` 함수를 그대로 복사하되, 반환값만 값을 반환하는 
부분마다 이를 `값 #:: f_stream_fun(i+1)` 식으로 변경해 스트림을 반환하게 한다.
2. 컴파일해보면 재귀호출 부분에서 타입 오류가 날 것이다. 이를 상황에 맞게 스트림에서 값을 가져오도록 바꾼다.
여기까지 해서 잘 해결될 수도 있다. 그렇지 않은 경우 다음 3번을 진행한다.
3. `f`가 `f(i)`를 계산하면서 i보다 작은 k에 대해 f(k)를 계산하는 부분이 있는 재귀함수였다면, 다음과 같은 식으로 
lazy val f_stream_val을 정의하는 곳 안에 앞의 f_stream_fun을 넣는다. 그리고 나서 `f_stream_fun` 내에서 i보다 
작은 k에 대해 f_stream_fun(k)를 호춣했던 부분을 f_stream_val을 참조하는 형태로 바꾼다.


```scala
lazy val f_stream_val:Stream[ReturnType] = {
  def f_stream_fun(i:Int) : Stream[ReturnType] = ...
  f_stream(0)
}
```

이렇게 하면 그리 어렵지 않게 스트림을 활용해 무한 시퀀스를 처리할 수 있을 것이다.

### 재귀 스트림 활용하기 : 스트림을 만들어내는 꼬리재귀 함수 정의

앞에서 `fibs`를 만들 때 원래의 재귀적 `fib()`함수를  사용하면, i번째 원소를 계산하기 위해 이전 스트림의 원소를
참조해야만 했다. 그런데, 실제로는 바로 앞 2개(`fib(i-1)`, `fib(i-2)`)만 활용하면 된다. 그리고 나서 다음번 원소를 
계산할때는 다시 `fib(i-1)+fib(i-2)`, `fib(i-1)`을 활용하게 된다. 

이를 꼬리재귀 스트림으로 만든다면 다음과 같다.
...
앞으로 설명시 a(n)은 어떤 시퀀스의 n번째 원소를 의미하고, A(n)은 [a(n),a(n+1),...]라는 무한 시퀀스를 
의미하는 것이다.

무한시퀀스에서 `a(n)`(타입은 T)을 만들 때 `k`개의 원소 `a(n-k),a(n-k+1),...,a(n-1)`에 `g()`를 적용하고, 
`a(n+1)`번째를 만들 때 또 `k`개의 원소 `a(n-k+1),a(n-k+2),...,a(n)`에 `g()`를 적용한다면,
k보다 큰 n에 대해서는 다음과 같이 쓸 수 있다.

```
lazy val a_stream = {
    def A_K_1(a_1, a_2, ..., a_k):Stream[T] = g(a_1, a_2, ..., a_k) #:: A_K_1(a_2,...,a_k,g(a_1, a_2, ..., a_k))
    a_1 #:: a_2 #:: ... #:: a_k #:: A_K_1(a_1, ..., a_k)
}
```

한번 이해하기 쉽게 피보나치 수열에 적용해보자. 

fibo의 경우 k=2, g = +라 할 수 있다.

```scala
lazy val fibs_stream = {
  def fibs_stream_k(a_1:Int, a_2:Int):Stream[Int] = (a_1+a_2) #:: fibs_stream_k(a_2,a_1+a_2)
  1 #:: 2 #:: fibs_stream_k(1,2)
}
```

### `Stream.zip`의 활용

무한시퀀스에서 `a(n)`을 만들 때 `2`개의 원소 `a(n-2),a(n-1)`에 `g()`를 적용하고, 
`a(n+1)`번째를 만들 때 또 `2`개의 원소 `a(n-1),a(n)`에 `g()`를 적용한다면 다음과 같은 짓도 가능하다.
(이를, 점화식 형태로, `a(n) = g(a(n-1),a(n-2))`이다.)

1. 무한 시퀀스 `A(i)`가 있다고 하자. (`i`번째 원소부터 시작하는 시퀀스)
2. 이제 그 시퀀스의 `tail`을 얻으면, 이는 `A(i+1)`과 같다. 즉, `A(i).tail = A(i+1)`이다.
3. 이 둘의 순서쌍 (즉, `PAIR(i) = [ (A(i)(0), A(i).tail(0)), (A(i)(1), A(i).tail(1)), ... ]` 을 얻을 수 이다.
이를 다르게 쓰면, `[ (a(i), a(i+1)), (a(i+1), a(i+2)), ...]` 이 된다.
4. 이 순서쌍에 `g`를 적용한다면, `PAIR(i).map(g) = [ g(A(i)(0), A(i).tail(0)), g(A(i)(1), A(i).tail(1)) ... ]` 이다.
이를 다르게 쓰면, `[ g(a(i), a(i+1)), g(a(i+1), a(i+2)), ...]`이다.
5. 그런데, 이 `PAIR(i).map(g)`를 잘 살펴보면, a(i+2) = g(a(i), a(i+1))이므로, 
`[ g(a(i), a(i+1)), g(a(i+1), a(i+2)), ...] = [a(i+2), a(i+3) ... ] = A(i+2)` 가된다. 

위 1~5에서 결국 다음과 같은 결론을 얻을 수 있다.(`S.zip(S2)`는 `(s(0),s2(0)),(s(0),s2(0)),...`을 얻는 함수이다.)

`A(i+2) = PAIR(i).map(g) = A(i).zip(A(i).tail).map(g)`

A(i)를 계속 연산하지 않고 가지고 있는다면 이 식을 사용해 무한급수를 쉽게 표현할 수 있다.
잠깐, 그거 어디서 많이 들어본 말인데? 맞다. 바로 그게 lazy evaluation이다.

이를 우리의 친구 피보나치에 적용해보면 다음과 같아진다.

```scala
lazy val fibo:Stream[Int] = 1 #:: 2 #:: fibo.zip(fibo.tail).map({ case (c1,c2) => c1 + c2 })
```

참 쉽죠~~잉? 이를 좀 더 확장하면 2개가 아닌 k개에 대해서도 가능할 지도 모른다.
(잘 모르겠다. 혹시 푼 분은 트랙백이나 이메일 주시길. enshahar 엣 지메일 닷 컴)
